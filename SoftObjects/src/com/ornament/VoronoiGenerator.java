...
import com.datatype.voronoi.Arc;
import com.datatype.voronoi.VorEvent;
import com.datatype.voronoi.VorEventSorter;
...
public class VoronoiGenerator {
    ...
    private Vector<Point> initPoints;  //the input points
    ...
    public VoronoiGenerator() {//constructor
    }

    public Pattern getEdges(Vector<Point> _points, int width, int height) {//returns a pattern corresponding to the voronoi diagram
        ...
        this.dcEdgeList = new DoublyConnectedEdgeList();
        
        this.faces = new DoublyConnectedEdgeList();
        ...
        pattern.addCopy(Geom.dCEdgeListToLines(dcEdgeList));
    ...
    private void handleSiteEvent(Point p) { // processes a site event
        ...
        if((root.isLeaf) && (root.site.getY() - p.getY() ==0)) // if root is a leaf and the current point following it is behind it (degenerate case)
          {
             ...
             /*Point fp = root.site;
              ...
              Point s = new Point((p.getX() + fp.getX())/2, height);
        ...
        Point start = new Point(p.getX(), vertexY);
    ...
    private void handleCircleEvent(VorEvent e) { //processes a circle event
        ...
        Point p = new Point(e.point.getX(), GetY(p1.site, e.point.getX()));
    ...
    private void finishEdge(Arc n) //recursively finishes all infinite edges in the tree
    {
        ...
        n.edge.end = new Point(mx, mx * n.edge.m + n.edge.b);
    ...
    private double GetY(Point p, double x) //find y intercept of parabola defined by p with x of new point;
    {
    ...
    private void checkCircle(Arc b) {//checks to see if breakpoints of 3 arcs converge to determine a potential circle event (the disappearing of the parabola)
        ...
        VorEvent e = new VorEvent(new Point(s.getX(), s.getY() - d), "circle");
    ...
    private Point getEdgeIntersection(DCHalfEdge a, DCHalfEdge b)//calculate intersection point between two edges
    {
