...
public class VoronoiGenerator {
	...
	public Vector<DCHalfEdge> getEdges(Vector<CompPoint> _points,DoublyConnectedEdgeList border, int width, int height){//returns a vector of edges corresponding to the voronoi diagram

		//set input points as initial points;
		this.initPoints = _points;
		...
		dcEdgeList = new DoublyConnectedEdgeList();
		...
		/*while(eventQueueCopy.size()>0) {

		    myParent.println(eventQueueCopy.poll().point.getY()); //debugging code
		...
		//return edges of the diagram;
		
	
		return dcEdgeList.edges;
	...
	private void handleSiteEvent(CompPoint p){ // processes a site event
		...
		/*if((root.isLeaf) && (root.site.getY() - p.getY() < 1)) // if root is a leaf and the current point following it is behind it (degenerate case)
		{
			myParent.println("root is leaf");
	...
	private DCHalfEdge checkEnd (DCHalfEdge edge){ //clips the end of a path so it is within the border
